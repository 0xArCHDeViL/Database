...
export function Heatmap({ store }: { store: TrackerStore }) {
  ...
  const minutesByDate = days.map((date) => ({ date, minutes: minutesForDate(store, date) }));
  const max = Math.max(1, ...minutesByDate.map((item) => item.minutes));
  return <div className="heatmap-wrap"><div className="heatmap-labels"><span>MINUTES / DAY</span><span>Last 12 weeks</span></div><div className="heatmap">{minutesByDate.map(({ date, minutes }) => { const level = minutes === 0 ? 0 : Math.min(4, Math.ceil((minutes / max) * 4)); return <span key={date} className={`heat-cell level-${level}`} title={`${date} · ${minutes} min`} />; })}</div><div className="heat-legend"><span>Less</span>{[0, 1, 2, 3, 4].map((level) => <span key={level} className={`heat-cell level-${level}`} />)}<span>More</span></div></div>;
}
...
export function ProgressLine({ store }: { store: TrackerStore }) {
  const data = Array.from({ length: 14 }, (_, index) => { const date = new Date(); date.setDate(date.getDate() - (13 - index)); const key = date.toISOString().slice(0, 10); return { key, value: minutesForDate(store, key) }; });
